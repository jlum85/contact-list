...
function run() {
  ...
  var _createApp = createApp(),
      contactsEl = _createApp.contactsEl; // Make sure the App is updated at when stating
  ...
  function createApp() {
    ...
    var _createAddForm = createAddForm(),
        addFormEl = _createAddForm.addFormEl;
...
function getElementByIdOrThrow(id) {
  ...
  if (!elem) {
    throw new Error("Cannot find element with id \"".concat(id, "\""));
  }
